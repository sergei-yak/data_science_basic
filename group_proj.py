...
df_binance = pd.read_csv('/Users/sergey_yak/Library/Mobile Documents/com~apple~CloudDocs/PYTHON/group_mini_project/data_science_basic/df_combined.csv') # this data is from binance exchange API btc to usdt price data
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        h_0 = torch.zeros(2 * self.num_layers, x.size(0), self.hidden_size)
        c_0 = torch.zeros(2 * self.num_layers, x.size(0), self.hidden_size)
